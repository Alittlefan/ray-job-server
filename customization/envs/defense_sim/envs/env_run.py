...
class DefenseGymV0(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        #场景类别
        self.scenario_id = self.config["scenario_id"]   #五种蓝方分布，与数量组合后组成50种场景

        #场景内蓝方兵力数量
        num_id = (self.scenario_id - 1) % 10
        ...
        self.total_soldier = self.config["total_soldier"][0] + num_id * 2
        self.total_vehicle = self.config["total_vehicle"][0] + num_id // 2
        self.total_enemy = self.total_soldier + self.total_vehicle
 
        self.blue_deploy_pos , self.all_blue_deploy_pos = self.generate_blue_deployment(self.total_soldier, self.total_vehicle, self.scenario_id)
    ...
    def get_observation(self):
        ...
        for pos in self.scenario.config.red_bigua_pos + self.scenario.config.red_bazhua_pos:
    ...
    def update_action_mask(self, action):
        ...
        # if bigua deploy,set
        if self._step_index == self.scenario.config.num_red_bigua - 1:       #壁挂部署完毕后，仅开放八爪actionmask
    ...
    def run_simulation(self):
        ...
        if self.sim_type == 1:
            ...
            infantry_deaths, vehicle_deaths, win = self.client.start_episode(self.scenario)
            is_done = True
    ...
    def calculate_escape_num(self):
        ...
        for group in self.scenario.blue_unit_target:
            for blue_unit in group:
                ...
                if(abs(lat - blue_unit['targets'][0]) <= 0.00105 and abs(lon - blue_unit['targets'][1]) <= 0.00043):
                    escape_num += 1
        return escape_num
