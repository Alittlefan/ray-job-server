...
from customization.envs.defense_sim.envs.data_types import (
    Formation,
    ScenarioConfig,
    TurretDeployment,
)
...
class ScenarioManager:
    ...
    def __init__(self, config: ScenarioConfig):
        ...
        # 一轮内红方部署情况
        self.deployed_turrets = set()

    def action_mapping(self, red_bigua_pos, red_bazhua_pos):
        self.conv_pos = {}
        index = 0
        # 映射bigua部署点
    ...
    def create_blue_units(self, blue_deploy_pos, blue_target_pos):
        ...
        for i in range(len(blue_deploy_pos)):
            ...
            for j in range(blue_deploy_pos[i][4]):
                ...
                idx += 1
            # if i %2!=0:
            for k in range(blue_deploy_pos[i][5]):
                blue_init_msg.append(
                    ObjectInitActionMsg(
                        timestamps=time.time(),
        ...
        for i, group in enumerate(blue_group_info):
            target_msg = SetGroupTargetInfoMsg(
                ...
                targetPos=Position3D(
                    blue_target_pos[0][1], blue_target_pos[0][0], blue_target_pos[0][2]
                ),
            )
    ...
    # #遍历全部蓝方部署
    # def create_blue_formations(self, selected_blue_deploy):
    #     """ 按照 `selected_blue_deploy` 创建蓝方编组 """
    #     self.blue_formations = []

    #     for deploy_data in selected_blue_deploy:
    #         lon, lat, alt, yaw, num_blue_man, num_blue_vehicle = deploy_data
    #         formation = Formation.create_formation(
    #             start=(lon, lat, alt, yaw),
    ...
    #         # 记录目标点
    #         group_target_info[j]["targets"] = (lon, lat)
    #         self.blue_unit_target.append(
    #             [
    #                 {"name": s, "targets": group_target_info[j]["targets"]}
    #                 for s in group_target_info[j]["members"]
    #             ]
    #         )
